...
def add_user_phrase(chat_id, phrase_id):
    ...
    try:
        ...
        print(f"❌ Ошибка добавления фразы: {e}")
        return False
...
def setup_bot_commands():
    ...
    try:
        ...
        print(f"❌ Помилка налаштування команд: {e}")
...
def api_save_progress():
    ...
    try:
        ...
        if chat_id:
            success = update_user_progress(chat_id, username, first_name, level, score, phrases)
            if success:
                return jsonify({'status': 'success', 'message': 'Прогрес збережено'})
            else:
                return jsonify({'status': 'error', 'message': 'Помилка збереження'}), 500
        else:
...
def api_add_phrase():
    ...
    try:
        ...
        if chat_id and phrase_id:
            success = add_user_phrase(chat_id, phrase_id)
            if success:
                return jsonify({'status': 'success', 'message': 'Фразу додано'})
            else:
                return jsonify({'status': 'error', 'message': 'Помилка додавання фрази'}), 500
        else:
...
def webhook():
    try:
        ...
        if 'message' in update:
            ...
            # Обработка текстовых кнопок и команд
            if text == '🎮 Грати':
...
🎮 Ігри: {progress['games_played']}
            ...
            elif text in ['/help', '/about', '/game', '/phrases', '/support']:
                # Простые ответы для остальных команд
                if text == '/help':
                    help_text = "📖 Використовуй кнопки меню для навігації!"
                    send_message(chat_id, help_text, reply_markup=create_reply_keyboard())
                elif text == '/about':
                    about_text = "💖 Love Number Puzzle - гра створена з любов'ю!"
                    send_message(chat_id, about_text)
                elif text == '/game':
                    keyboard = create_main_keyboard()
                    send_message(chat_id, "🎮 Запускай гру!", keyboard)
                elif text == '/phrases':
                    progress = get_user_progress(chat_id)
                    phrases_count = progress['phrases_found'] if progress else 0
                    send_message(chat_id, f"💖 Знайдено фраз: {phrases_count}/40")
                elif text == '/support':
                    send_message(chat_id, "🆘 Допомога: @bergmann1")
